...
setwd("C:/Users/J.ABREU/OneDrive - ESPM/4° SEMESTRE/R/Rstudio/Aulas/Aula09")
library(readxl)
dados = read_excel("Aula 09 - regressão simples.xlsx", sheet = "TV")
...
library(ggplot2)
set.seed(123) # guarda o gerador de número aleatório
ggplot(dados, aes(x = SAC, y = CONFIANÇA)) +
...
setwd("C:/Users/J.ABREU/OneDrive - ESPM/4° SEMESTRE/R/Rstudio/Aulas/Aula09")
library(readxl)
dados = read_excel("Aula 09 - regressão simples.xlsx", sheet = "Paraguai")

```

a) Calcule a correlação para o problema e descreva a relação entre papel-moeda 
emitido e depósitos no Banco Central do Paraguai  através de um gráfico.
...
O gráfico mostra que os depósitos aumentam à medida que a emissão de moeda também aumenta.
...
O valor da correlação indica forte associação entre as variáveis

```{r echo = FALSE, message = FALSE, warning = FALSE}
modelo = lm(y ~ x) # -1 exclui o intercept
resumo = summary(modelo)
...
kable(round(resumo$coefficients,3), caption = "Equação de regressão")

# Para obter os LI e LS dos coeficientes com duas casas decimais
kable(round(confint(modelo, level = 0.95),2), caption = "Limites")
```

Como valor - p referent ao intercepto é maior do que 5%, precisamos rodar um novo modelo, colocando -1 dentro da modelagem.
...
O modelo preditivo final não apresenta o coeficiente linear. Ele é dado por:
$$ Depósitos = 0,57 * Emissão. $$
