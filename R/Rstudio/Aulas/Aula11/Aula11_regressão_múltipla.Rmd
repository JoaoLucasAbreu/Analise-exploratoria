...
Marginalmente, as variáveis IPCA e PIB são irrelevantes para a predição do volume de negociações no mercado à vista (valores-p marginais são maiores do que 10%). Já as variáveis JUROS e EMBI são significativas marginalmente (valores-p menores do que 10%).
...
intervalo = confint(modelo3, level = 0.90)
...
```{r}
library(rgl)
scatter3d(VOLUME~EMBI+JUROS, data=dados)
```
